                    ...
                    @else
                        <ul style="padding: 0;margin: 0">
                            @foreach($selected_values as $selected_value)
                                @if($options->model == 'App\OfferFile')
                                    <a href="{{'/storage/'.$selected_value }}" data-toggle="lightbox" data-gallery="example-gallery">
                                        <div class="image-area">
                                            <img style="margin-right: 15px" src="{{ ImageManager::getImagePath(public_path() .'/storage/'.$selected_value, 250, 150, 'crop') }}" />
                                            <a class="download-image" onclick="downloadURI('{{'/storage/'.$selected_value }}', 'img')"><i class="voyager-download"></i></a>
                                        </div>
                                    </a>
                                @elseif($options->model == 'App\OfferVideo')
                                    <a href="{{'/storage/'.$selected_value }}" data-toggle="lightbox" data-gallery="example-gallery">
                                        <video src="{{'/storage/'.$selected_value }}" width="250" height="150" controls style="margin-right: 15px" ></video>
                                    </a>
                                @else
                ...
                @if(isset($query))
                    <ul style="padding: 0;margin: 0">
                        @foreach($query as $query_res)
                            @if($options->model == 'App\OfferFile')
                                    <a href="{{'/storage/'.$query_res->{$options->label} }}" data-toggle="lightbox" data-gallery="example-gallery">
                                    <div class="image-area">
                                        <img style="margin-right: 15px"  src="{{ ImageManager::getImagePath(public_path() .'/storage/'.$query_res->{$options->label}, 250, 150, 'crop') }}" />
                                        <a class="remove-image" href="#" data-id="{{ $query_res->id }}" style="display: inline;">&#215;</a>
                                        <a class="download-image" onclick="downloadURI('{{'/storage/'.$query_res->{$options->label} }}', 'img')"><i class="voyager-download"></i></a>
                                    </div>
                                    <a/>
                                    @elseif($options->model == 'App\OfferVideo')
                                        <a href="{{'/storage/'.$query_res->{$options->label} }}" data-toggle="lightbox" data-gallery="example-gallery">
                                            <div class="video-area">
                                                <video style="margin-right: 15px" controls width="350" height="200">
                                                    <source src="{{ '/storage/'.$query_res->{$options->label} }}" type="video/mp4">
                                                </video>
                                                <a class="remove-video" href="#" data-id="{{ $query_res->id }}" style="display: inline;">&#215;</a>
                                            </div>
                                            <a/>
                                            @else
                        ...
                        @endforeach
                            @include('voyager::formfields.multiple_images', ['dataTypeContent' => $dataTypeContent])
                    </ul>
